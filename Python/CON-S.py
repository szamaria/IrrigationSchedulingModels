"""
 SCENARIO 5: 
Essentially the same as McInerney's (2018) SU-C scenario. Crop water requirement per crop per HRU is calculated by summing AET. Then, CWR is divided by each hru's growing season. The daily amount of water is applied
to the hru, daily.
 """
...
directory = "c:/phd_arcswat/projects/bigcreek_2006-2019/python scripts/IRRIGATION_2023_24/Scenario_5/mgt_files" # sets mgt files as directory for appending later
...
# 4. Calculate annual crop water requirements per crop, per soil type (per hru)
                if year >= 2011:  #start of calibration period
                    ...
                    gs = end_date - start_date
                    # print(gs.days)
                    # print(cwr)
                    irr_amt = cwr/gs.days #daily irr amt
                    # print(irr_amt)
