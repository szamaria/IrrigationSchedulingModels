# """
# SCENARIO 3: 
# ipped and the skipped irrigation water is added to a cumulative pool. At the end of each year, accumulated skipped irrigation is
# retroactively applied to irrigation events from July-September, evenly.
# """
import os
import pandas as pd
import numpy as np
...
directory = "c:/phd_arcswat/projects/bigcreek_2006-2019/python scripts/IRRIGATION_2023_24/Scenario_3/mgt_files" # sets mgt files as directory for appending later
...
sol_directory = "c:/phd_arcswat/projects/bigcreek_2006-2019/python scripts/IRRIGATION_2023_24/Scenario_3/sol_files" # sets sol files as directory
# sol_files = [f for f in listdir(sol_directory) if isfile(join(sol_directory, f))]
...
# 2. Define crops and dates   
        if crop_key in crops.keys(): # only run if crop is in list (tobc, corn, soyb)
            ...
            day_count = 0
            year_break = 2007
            etsum = hrus.groupby(['HRU', 'YEAR'])['ETmm'].sum().reset_index()
...
# 4. Calculate annual crop water requirements per crop, per soil type (per hru)
                if year >= 2011:  #start of calibration period
                    cwr = etsum.query(f'YEAR == {year}')["ETmm"].iloc[0]
                    # print(cwr)
                    irrevent_no = cwr/crop["id"] #typical no of irr events/year
                    # print(irrevent_no) #Eventually want to compare this to number of simulated irr events/hru

# # 4. Calculate allowable water depletion threshold per HRU
#                         #First define available water capacity at the root zone. Take average of all 5 AWC values from every .sol file. Move this outside of date loop? 
#                         #(SOL_AWC values from .sol)
...
# # 4. Calculate total irrigation pools at beginning of every year             
                    if date >= start_date and date <= end_date:
                        SWend = hrus.query(f'MON == {month} and DAY == {day} and YEAR == {year}')["SW_ENDmm"].iloc[0]
                        # print(SWend)

                    
# 5. Apply irrigation based on algorithm 
                        if (SWend < AWD) and ((AWC - SWend) > crop["id"]): #changed Feb26
                            irr_amt = crop["id"]
                            crop["gw"] =  round(irr_amt * 0.73, 2) 
                            crop["sw"] = round(irr_amt * 0.27, 2)  #added lines 203 and 204 Feb 23
                            outputvar_rows.append([day, month, year, subbasin, hru, AWC, AWD, SWend, irr_amt])
                            print("this is the current day" + str(day))
                            print("this is the current month" + str(month))
                            print("this is the current year" + str(year)) 
                            print("this is the current HRU" + str(hruno)) 
                            print("this is AWD:" + str(AWD))
                            print("this is SWend:" + str(SWend))
                            print("this is AWC:" + str(AWC))
                            print("this is irramt:" + str(irr_amt)) 
                        elif (SWend < AWD) and ((AWC - SWend) <= crop["id"]):    #Changed Feb26 
                            ...
                            crop["sw"] = 0
                        # print(crop["gw"])
                            outputvar_rows.append([day, month, year, subbasin, hru, AWC, AWD, SWend, ""])
...
outputvars = pd.DataFrame(outputvar_rows,columns = output_columns)
outputvars.to_csv("irr3_output.csv", index = False)
print("done all")
